...
Embaixo do header está o código para emis.R. `pol` sao os poluentes, `metadata` sao as categorias veiculares, `mileage` sao as milhas percorridas por categoria veicular, `tfs` sao os perfis de uso por categoria veicular, `veh` sao as frotas veiculares, `met` sao os metodos de emissao, `net` e a rede, `lkm` sao os km percorridos por segmento de rede, `scale` e a escala de emissao, `verbose` e para mostrar informacoes, `s` e o fator de escala, `fuel` e o combustivel, `IM` e para usar os fatores de emissao do IM, `pol` e os poluentes a serem estimados.

Neste caso nao e usado, que e o default.
