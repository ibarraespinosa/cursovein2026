...
nonmhc_m <- emi[
  ...
  by = .(pollutant, region, month)
]

nmhc_m <- emi[
  pollutant %in% c("NMHC"),
  sum(emissions, na.rm = T),
  by = .(type_emi, pollutant, region, fuel, month)
]
...
for (k in 1:12) {
  ...
  do.call(
    "rbind",
    pbapply::pblapply(seq_along(roads), function(i) {
      x <- st_read(roads[i], quiet = T)
      x$region <- na_roads[i]
      x$length <- st_length(x)
      x$lengthHDV <- ifelse(
        x[[osm_name]] %in% c("tertiary", "secondary"),
        0,
        x$length
      )
      ...
      for (j in seq_along(dtpol)) {
        if (dtpol[j] %in% c("NO", "NOx", "NO2")) {
          ...
          x[[dtpol[j]]] <- dt[pollutant == dtpol[j]]$V1 *
            x$lengthHDV /
            sum(x$lengthHDV)
        } else {
          x[[dtpol[j]]] <- dt[pollutant == dtpol[j]]$V1 *
  ...
  saveRDS(
    emis_street,
    paste0("post/streets/emis_street_", sprintf("%02d", (1:12)[k]), ".rds")
  )
}
